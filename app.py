...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v11.0", layout="wide")
...
PARAM_GROUPS = {
    ...
    "Control Kubios": ["Entropy_lambda", "Entropy_m", "Entropy_r_factor", "Entropy_SD_ms", "Entropy_r_ms", "Entropy_N", "DFA_alpha1_range", "DFA_alpha2_range", "RQA_threshold_SD", "RQA_emb_dim", "MSE_zero_policy", "SampEn_Theiler"],
}
...
def sample_entropy_common(rr_entropy, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None, zero_policy=None, theiler_window=None):
    """
...
def rcmse_common(rr_entropy, scales=KUBIOS_MSE_MAX_SCALE, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None, theiler_window=0):
    """
    ...
    for scale in range(1, int(scales) + 1):
        if scale <= 1:
            out[f"MSE{scale}"] = _sample_entropy_core(x, m=m, r=r, zero_policy="nan", theiler_window=theiler_window)
            continue
        ...
        for offset in range(scale):
            ...
            B, A = _sample_entropy_counts_from_series(cg, m=m, r=r, theiler_window=theiler_window)
...
def mse_common(rr_entropy, scales=KUBIOS_MSE_MAX_SCALE, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None, zero_policy="nan", theiler_window=None):
    """
    MSE v10.6.
    ...
    for scale in range(1, int(scales) + 1):
        ...
        if len(cg) <= m + 2:
            ...
            out[f"MSE{scale}"] = _sample_entropy_core(cg, m=m, r=r, zero_policy=zero_policy, theiler_window=theiler_window)

    # MSE1 se mantiene igual a SampEn clásica principal.
    out["MSE1"] = _sample_entropy_core(x, m=m, r=r, zero_policy=zero_policy, theiler_window=theiler_window)
    return out
...
def mse_metrics(rr, scales=20, max_scale=None, m=2, r=0.2, zero_policy=None, theiler_window=None):
    """
    ...
    return mse_common(rr, scales=scales, m=m, r_factor=r, r_reference=rr, zero_policy=zero_policy, theiler_window=theiler_window)
...
def sample_entropy_diagnostic_rows(rr_entropy, scales=20, m=2, r_factor=0.2, r_reference=None, theiler_window=0):
    ...
    for scale in range(1, int(scales) + 1):
        ...
        B, A = _sample_entropy_counts(cg, m=m, r=r, theiler_window=theiler_window)
...
def calculate_all(rr, include_rqa=True, include_hvg=False, mse_zero_policy=None, theiler_window=None):
    """
...
def calculate_record(rr, windows, active_phases, min_rr, include_rqa, include_hvg=False, mse_zero_policy=None, theiler_window=None):
    rows, segments, valid = [], {}, {}
    ...
    for ph in PHASES:
        ...
        if valid[ph]:
            res = calculate_all(seg, include_rqa=include_rqa, include_hvg=include_hvg, mse_zero_policy=mse_zero_policy, theiler_window=theiler_window)
            res["Fase"] = ph
            rows.append(res)
...
st.title("VRC / HRV RRi Analyzer Pro v11.0")
...
for rec, data in record_data.items():
    ...
    df, segs, valid = calculate_record(data["rr"], w, active_phases, min_rr, include_rqa, include_hvg=include_hvg, mse_zero_policy=st.session_state.get("mse_zero_policy", "nan"), theiler_window=st.session_state.get("sampen_theiler_window", 0))
